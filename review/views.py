...
def summary(request, username):
    ...
    review_milestones = ReviewMilestone.objects.all().order_by('-created')
    for review_milestone in review_milestones:
        ...
        comments = Comment.objects.filter(author=participant) \
                          .filter(chunk__file__submission__milestone__id = review_milestone.submission_milestone.id).select_related('chunk')
        review_data = []
        ...
        votes = Vote.objects.filter(author=participant) \
                    .filter(comment__chunk__file__submission__milestone__id = review_milestone.submission_milestone.id) \
