...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Delete all submissions with no files
        orm.Submission.objects.annotate(file_count=models.Count('files')).filter(file_count=0).delete()

        # Give milestones to submissions with files but no milestone
        for submission in orm.Submission.objects.filter(milestone=-1):
