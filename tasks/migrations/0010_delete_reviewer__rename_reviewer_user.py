...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'Task.reviewer_user'
        db.alter_column(u'tasks_task', 'reviewer_user_id', self.gf('django.db.models.fields.related.ForeignKey')(null=True, to=orm['auth.User']))
        # Adding unique constraint on 'Task', fields ['chunk', 'reviewer_user']
        db.create_unique(u'tasks_task', ['chunk_id', 'reviewer_user_id'])


        # Renaming field 'Task.reviewer_user' to 'Task.reviewer'
        db.rename_column(u'tasks_task', 'reviewer_user_id', 'reviewer_id')


    def backwards(self, orm):
        # Renaming field 'Task.reviewer' to 'Task.reviewer_user'
        db.rename_column(u'tasks_task', 'reviewer_id', 'reviewer_user_id')

        # Removing unique constraint on 'Task', fields ['chunk', 'reviewer_user']
        db.delete_unique(u'tasks_task', ['chunk_id', 'reviewer_user_id'])

        # Adding field 'Task.reviewer'
        db.add_column(u'tasks_task', 'reviewer',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, null=True, related_name='tasks', to=orm['accounts.UserProfile']),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'Task.reviewer_user'
        #raise RuntimeError("Cannot reverse this migration. 'Task.reviewer_user' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration
        # Changing field 'Task.reviewer_user'
        db.alter_column(u'tasks_task', 'reviewer_user_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User']))
    ...
    models = {
        ...
        u'tasks.task': {
            'Meta': {'unique_together': "(('chunk', 'reviewer_user'),)", 'object_name': 'Task'},
